  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 ce-fade-up">
        <div>
          <h1 className="text-2xl font-semibold text-[#0b2338]">Materiais</h1>
          <p className="text-sm text-slate-500 mt-1">{db.materials.length} materiais cadastrados · valores demonstrativos e editáveis</p>
        </div>
      ...
      <div className="ce-card-hover bg-white border border-slate-200 rounded-xl overflow-x-auto ce-fade-up ce-fade-up-1">
        <table className="w-full text-sm min-w-[720px]">
          ...
          <tbody>
            {filtered.map(m => (
              <tr key={m.id} className="border-b border-slate-200 last:border-0 hover:bg-slate-100">
                <td className="px-5 py-3 text-slate-500">{m.codigo}</td>
                <td className="px-5 py-3 text-[#0b2338]">{m.nome}</td>
                <td className="px-5 py-3 text-slate-600">{m.categoria}</td>
                <td className="px-5 py-3 text-slate-600">{m.unidade}</td>
                <td className="px-5 py-3 text-slate-600">{formatMoney(m.preco_custo)}</td>
                <td className="px-5 py-3 text-[#0b2338] font-medium">{formatMoney(m.preco_venda)}</td>
                <td className="px-5 py-3">
                  <span className={`inline-flex items-center gap-1 text-xs ${m.estoque_atual <= m.estoque_minimo ? 'text-amber-400' : 'text-slate-600'}`}>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50" onClick={() => setOpen(false)}>
          <form onClick={e => e.stopPropagation()} onSubmit={submit} className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-lg space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-[#0b2338] font-semibold">Novo material</h2>
            ...
            {Number(form.preco_venda) > 0 && (
              <p className="text-xs text-slate-400 -mt-2">
                Margem estimada: <span className="text-[#00B4E5] font-medium">{((( Number(form.preco_venda) - Number(form.preco_custo||0)) / Number(form.preco_venda)) * 100).toFixed(1)}%</span>
              </p>
            )}
            <div className="grid grid-cols-2 gap-3">
            ...
            <button type="submit" className="w-full bg-[#00B4E5] text-[#0b2338] font-semibold rounded-lg py-2.5 text-sm hover:bg-[#0069A8]">Salvar material</button>
          </form>
        </div>
      )}
    </div>
