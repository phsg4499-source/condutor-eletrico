  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 ce-fade-up">
        <div>
          <h1 className="text-2xl font-semibold text-[#0b2338]">Serviços</h1>
          <p className="text-sm text-slate-500 mt-1">{db.services.length} serviços cadastrados · valores editáveis</p>
        </div>
      ...
      <div className="ce-card-hover bg-white border border-slate-200 rounded-xl overflow-x-auto ce-fade-up ce-fade-up-1">
        <table className="w-full text-sm min-w-[640px]">
          ...
          <tbody>
            {filtered.map(s => (
              <tr key={s.id} className="border-b border-slate-200 last:border-0 hover:bg-slate-100">
                <td className="px-5 py-3 text-slate-500">{s.codigo}</td>
                <td className="px-5 py-3 text-[#0b2338]">{s.nome}</td>
                <td className="px-5 py-3 text-slate-600">{s.categoria}</td>
                <td className="px-5 py-3 text-slate-600">{s.unidade}</td>
                <td className="px-5 py-3 text-slate-600">{formatMoney(s.custo_mao_obra)}</td>
                <td className="px-5 py-3 text-[#0b2338] font-medium">{formatMoney(s.valor_padrao)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50" onClick={() => setOpen(false)}>
          <form onClick={e => e.stopPropagation()} onSubmit={submit} className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-lg space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-[#0b2338] font-semibold">Novo serviço</h2>
            ...
            <button type="submit" className="w-full bg-[#00B4E5] text-[#0b2338] font-semibold rounded-lg py-2.5 text-sm hover:bg-[#0069A8]">Salvar serviço</button>
          </form>
        </div>
      )}
    </div>
