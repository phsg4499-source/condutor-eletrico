...
export default function Agenda() {
  ...
  async function handleDelete(id: string, titulo: string) {
    ...
    if (!result.ok) {
      toast.show(result.error ?? 'Não foi possível excluir o compromisso.', 'warning');
      return;
    }
    toast.show('Compromisso excluído.', 'info');
  }

  async function handleToggleConcluido(c: Compromisso) {
    const result = await updateCompromisso(c.id, { status: c.status === 'concluido' ? 'agendado' : 'concluido' });
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <CompromissoForm
          ...
          onSave={async (data) => {
            if (editingId) {
              const result = await updateCompromisso(editingId, data);
              if (!result.ok) {
                toast.show(result.error ?? 'Não foi possível atualizar o compromisso.', 'warning');
                return;
              }
              toast.show('Compromisso atualizado.');
            } else {
              const result = await addCompromisso(data as Omit<Compromisso, 'id' | 'organization_id' | 'created_at' | 'updated_at'>);
              if (!result.ok) {
                toast.show(result.error ?? 'Não foi possível salvar o compromisso.', 'warning');
                return;
              }
...
function CompromissoForm({ compromisso, prefill, defaultDate, orcamentistasAtivos, clients, compromissos, onClose, onSave }: {
  ...
  onClose: () => void;
  onSave: (data: Partial<Compromisso>) => void | Promise<void>;
}) {
