...
export default function PublicProposal() {
  ...
  async function confirmarResposta() {
    ...
    try {
      if (!isSupabaseConfigured) {
        const result = await setBudgetStatus(vm.budget.id, respondendo === 'solicitou_alteracao' ? 'revisao_solicitada' : respondendo);
        if (!result.ok) {
          toast.show(result.error ?? 'Não foi possível registrar sua resposta. Tente novamente.', 'warning');
          return;
        }
