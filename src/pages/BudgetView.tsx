...
export default function BudgetView() {
  ...
  async function handleConvert() {
    const result = await convertBudgetToServiceOrder(budget!.id);
    if (!result.ok || !result.order) {
      toast.show(result.error ?? 'Não foi possível converter em ordem de serviço.', 'warning');
      return;
    }
    toast.show('Orçamento convertido em ordem de serviço!');
    navigate(`/app/ordens-servico/${result.order.id}`);
  }

  async function handleStatusChange(newStatus: BudgetStatus) {
    const result = await setBudgetStatus(budget!.id, newStatus);
    if (!result.ok) {
      toast.show(result.error ?? 'Não foi possível atualizar o status do orçamento.', 'warning');
      return;
    }
    toast.show(result.warning ?? 'Status atualizado.', result.warning ? 'warning' : 'info');

    // Quando a aprovação acontece aqui dentro (você mudando o status), a Agenda abre sozinha, com
    // os dados já preenchidos — é o fluxo que você pediu. Quando o CLIENTE aprova pelo link
    // público (WhatsApp), essa navegação automática não existe (são sessões/dispositivos
    // diferentes) — nesse caso o banner "Agendar execução" abaixo cobre o mesmo agendamento,
    // disponível sempre que o orçamento estiver aprovado e ainda sem compromisso vinculado.
    if (newStatus === 'aprovado' || newStatus === 'aprovado_parcialmente') {
      handleAgendarExecucao();
    }
  }
      ...
      <div className="bg-[#16181d] border border-white/5 rounded-xl p-5 space-y-4">
        ...
        {(budget.status === 'aprovado' || budget.status === 'aprovado_parcialmente') && budgetPayments.length === 0 && (
          <div className="bg-[#f5c518]/10 border border-[#f5c518]/20 rounded-lg px-4 py-3 text-xs text-amber-300 space-y-2">
            ...
            <button
              onClick={async () => {
                const result = await applySaldoReconciliation(db.payments, { budget, novoSaldo: totals.totalVenda }, { addPayment, updatePayment, deletePayment });
                if (!result.ok) {
                  toast.show(result.error ?? 'Não foi possível lançar o valor a receber.', 'warning');
                  return;
                }
