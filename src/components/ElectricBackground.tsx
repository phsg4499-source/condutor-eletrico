// Fundo animado de "correntes elétricas" para telas de alto impacto (login).
// Composição: grade sutil em movimento + trilhas de circuito com pulso de energia percorrendo
// cada linha, em looping contínuo e discreto (opacidade baixa, sem exagero).
export default function ElectricBackground() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
      <div className="ce-grid-bg absolute inset-0 opacity-60" />
      <svg viewBox="0 0 1200 800" preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
        <defs>
          <linearGradient id="ce-line-grad" x1="0" y1="0" x2="1" y2="0">
        ...
        {lines.map((line, i) => (
          <path
            ...
            stroke="url(#ce-line-grad)"
            strokeWidth={i === 2 ? 2 : 1.4}
            style={{ opacity: line.opacity, animationDuration: `${line.duration}s`, animationDelay: `${line.delay}s` }}
          />
        ))}
      </svg>
