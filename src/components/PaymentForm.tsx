 ...
 */
export async function applySaldoReconciliation(
  payments: Payment[],
  { budget, novoSaldo, vencimento }: SaldoReconciliacao,
  actions: {
    addPayment: (data: PaymentInput) => Promise<{ ok: boolean; error?: string }>;
  ...
  if (novoSaldo <= 0) {
    if (existente) return actions.deletePayment(existente.id);
    return { ok: true };
  }
  if (existente) {
    return actions.updatePayment(existente.id, { valor: novoSaldo, ...(vencimento ? { vencimento } : {}) });
  }
  return actions.addPayment({
...
export function PaymentForm({ budgets, clients, payments, lockedBudget, onClose, onSave }: {
  ...
  lockedBudget?: Budget;
  onClose: () => void;
  onSave: (data: PaymentInput, saldo?: SaldoReconciliacao) => void | Promise<void>;
}) {
